...
const WalletScreen = ({  navigation}) => {
  ...
  const pullPaymentHistory = async (start) =>{
    //console.log(start)
    const histwallter = await getPaymentHistory(data?.user?.UserID,start,user);
    setWalletTransaction(histwallter.data.reverse() ?? [])
    await AsyncStorage.setItem(`payment_${start}`, JSON.stringify(histwallter.data)); // Cache it
  }
  ...
  useEffect(()=>{
    ...
    const pullSettings = async () =>{
        ...
        const cachedData = await AsyncStorage.getItem(`payment_${today}`);
        if (cachedData) {
          setWalletTransaction(JSON.parse(cachedData).reverse()); // Show cached data immediately
        }
  ...
  const onChange = (event, selectedDate) => {
    ...
    setDate(currentDate);
    setFilter(true)
    var formattedDate = new Date(currentDate).toISOString().split("T")[0]; //date selected in the calendar component
    pullPaymentHistory(formattedDate);
  };
