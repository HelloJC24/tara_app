...
const WalletScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const pullSettings = async () => {
      ...
      if (
        sets.message == "No settings found for the provided UserID or TaraID."
      ) {
        //create
    ...
    const pullPaymentHistory = async () => {
      const histwallter = await getPaymentHistory(
        data?.user?.UserID,
        "2025-01-29",
        user
      );
        ...
        flex flex-row items-center justify-between"
          ...
          <View className="w-full px-2">
            ...
            <View className="w-full pb-8">
              <TouchableOpacity
                onPress={() => setActiveWithdraw(true)}
                className=" bg-slate-100 p-3 rounded-xl flex gap-x-4 flex-row items-center justify-center"
              >
                <Svg
                  xmlns="http://www.w3.org/2000/svg"
                  width={20}
                ...
                <Text className="text-base text-blue-500">Withdraw funds</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const WithdrawScreen = ({ navigation, close }) => {
          ...
          <View className="py-4">
            <View className="flex gap-y-4">
              ...
              <View className="px-4 flex-row items-center gap-x-2">
                ...
                <ParagraphText fontSize="sm" textColor="text-neutral-700">
                  I confirm that I reviewed and double check the account details
                  is correct.
                </ParagraphText>
              </View>
            </View>
            ...
            <View className="px-2">
              <View className="w-full py-6">
                <Text className="text-2xl font-semibold">Withdraw Options</Text>
                <Text className="text-base">
                  See our available payment gateway
                </Text>
              </View>

              <View>
                {withdrawOptions?.map(({ id, provider, endpoint, status }) => {
                  return (
                    <View
                      key={id}
                      className="w-full py-2 flex-row justify-between items-center"
                    >
...
const RadioButton = ({ active, onPress }) => {
  return (
    <TouchableOpacity
      className={`w-6 h-6 rounded-full p-[2px] ${
        active ? "border border-blue-500 " : "border border-slate-300"
      }`}
      onPress={onPress}
    >
      <View
...
const SendOrTransferScreen = ({ realtiming, rep, balance, openQR, close }) => {
  ...
  const checkRep = async () => {
    ...
    if (cehckas.message == "User found.") {
      ...
      toast(
        "error",
        "No Tara account associated with this information was found."
      );
    }
  };
        ...
        <View>
          ...
          <View className="w-full flex gap-y-4 px-6 py-10">
            <ParagraphText
              ...
              padding="px-4"
            >
              By clicking “Send”, you confirm that all details are correct and
            ...
            {allowed ? (
              <Button onPress={() => startSending()}>Send</Button>
            ) : (
